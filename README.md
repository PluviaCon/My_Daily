...
[odoo文档](./Odoo/Odoo.md)中有具体方法

#### Dockerfile

包含用户定义指令的文本文件,用于组装Docker镜像,通过具体的指令来执行相应的操作.

- `FROM` 定义所需镜像名称及版本.
- `LABEL` 位镜像添加额外信息.
- `RUN` 用于和镜像交互,来安装依赖或软件或是运行脚本,命令.一条`RUN`命令可以使用`&&` 和 `\` 来分隔带多行
- `COPY & ADD` 两个类似的命令,但是有很多区别.`COPY`命令是从宿主机拷贝到容器镜像内,`ADD`命令多用于数据压缩包的解压.
- `EXPOSE` 在docker镜像被执行后,运行时暴露的端口和协议.
- `ENTRYPOINT` 配置容器启动后执行的命令,并且不可被`docker run` 提供的参数覆盖,并且一个dockerfile只能有一个`ENTRYPOINT`,制定多个只有一个有效
